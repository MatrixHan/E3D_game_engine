...
namespace E3D
{
	...
	ETank::ETank(const EString &name, const EString &meshName, EGameManager *gameMgr)
		: mGameMgr(gameMgr), mSceneManager(gameMgr->getSceneManager()), mSpeed(0.75f),
		mFireDir(0, 0, -1), mLastFire(0), mBulletType(BULLET_ROCKET),
		mMoveVec(0, 0, 0), mColOffset(3.5f), mTankType(TANK_PALYER)
	{
		mMaxLive = 100;
		mCurrentLive = 0;
		mFireFrequency = 8;
		...
		mName = name;
		mBody = mSceneManager->createMesh(name + "_Body", meshName + "_Body.mesh");
		mHead = mSceneManager->createMesh(name + "_Head", meshName + "_Head.mesh");
	}
	...
	ETank::~ETank()
	{
		mSceneManager->destroyMesh(mName + "_Body");
		mSceneManager->destroyMesh(mName + "_Head");
	}

...
namespace E3D
{
	...
	EUShort ReadChunk(std::ifstream &in)
	{
		...
		EUInt lenght;
		in.read((EChar*)&lenght, sizeof(EUInt));
	...
	{
		Log("EUShort:"); Log(IntToString(sizeof(EUShort)).c_str());
		Log("EUInt:"); Log(IntToString(sizeof(EUInt)).c_str());
		Log("EBool:"); Log(IntToString(sizeof(EBool)).c_str());
		std::ifstream in;
		// 模型文件, 首先获取默认路径

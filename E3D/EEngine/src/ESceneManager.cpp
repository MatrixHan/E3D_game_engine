...
namespace E3D
{
	...
	{
		for (MeshItr iter = mMeshes.begin(); iter != mMeshes.end(); ++iter)
			SafeDelete(iter->second);
		for (MeshItr iter = mUsedMesh.begin(); iter != mUsedMesh.end(); ++iter)
			SafeDelete(iter->second);
	...
	{
		MeshItr iter = mMeshes.find(name);
		if (iter == mMeshes.end())
		{
			EMesh *mesh = NULL;

			MeshItr meshItr = mUsedMesh.find(meshName);
	...
	void ESceneManager::update()
	{
		...
		for (MeshItr iter = mMeshes.begin(); iter != mMeshes.end(); ++iter)
		{
			...
			if (iter->second->isVisible() && !Cull_Object4D(iter->second->getObject(), mCamera->getFrustum(), CULL_BOX))
			{
				...
				Insert_Object4D_To_RenderList4D(mRenderList, iter->second->getObject(),
